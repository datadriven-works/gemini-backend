...
resource "google_service_account" "gemini-backend-sa" {
  account_id   = "gemini-backend-cf-sa"
...
resource "google_project_iam_member" "iam_permission_looker_aiplatform" {
  ...
  member  = format("serviceAccount:%s", google_service_account.gemini-backend-sa.email)
}
...
resource "google_secret_manager_secret_iam_binding" "vertex_cf_auth_token_accessor" {
  ...
  members   = [
    "serviceAccount:${google_service_account.gemini-backend-sa.email}",
  ]
}
...
resource "google_artifact_registry_repository" "default" {
  repository_id = "gemini-backend-repo"
  location      = var.deployment_region
  project       = var.project_id
  format        = "DOCKER"
}
...
resource "google_cloudfunctions2_function" "default" {
  ...
  service_config {
    ...
    service_account_email          = google_service_account.gemini-backend-sa.email
  }
}
